...
4. **脚本校验服务**
   ...
   - 控制执行超时与资源限制，避免长时间阻塞。
5. **Schema 校验逻辑**
   - 校验字段唯一性、类型与组件映射关系、validator 格式。
   - 维护内置校验规则列表，供前端/后端共同引用。
6. **回测任务联动**
...
5. **轻量 Schema Renderer**
   - 参数/因子共用组件；支持字段增删改、排序、校验提示。
   - 内置组件覆盖 `input`、`number`、`select`、`checkbox`、`multi-select` 等。
   - 支持校验规则选择与表达式填写，并给出格式说明。
6. **版本列表与操作**
