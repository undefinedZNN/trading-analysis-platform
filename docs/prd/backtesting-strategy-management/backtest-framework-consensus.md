...
### 9.1 目标
- 为回测框架提供可重复的集成测试与回归用例，覆盖数据管线、策略沙箱、风控规则、执行撮合、交易账簿、快照恢复等关键路径。
- 作为开发示例，帮助新策略快速理解 API 与事件流。
- 支持 CI/CD：每次框架改动可运行测试策略，若结果偏离预期则阻止发布。

### 9.2 策略目录与约定
- 测试策略存放于 `tests/strategies/`（或同等位置），每个策略一个子目录，结构类似生产策略但包含预置配置与断言。
- 每个测试策略需要提供：
  - `parameters` / `customFeatures`（若需要）；
  - 预置数据集或数据源配置；
  - 期望输出（PnL、交易量、风险事件数等）；
  - 校验脚本（可以是 TypeScript/脚本或 JSON 断言），在测试流程中自动比对。
- 测试运行工具（CLI 或脚本）负责载入策略、执行回测、收集结果、比较断言。

### 9.3 测试策略类型

| 名称 | 作用 | 重点验证 | 预期断言示例 |
|------|------|----------|--------------|
| `PriceEcho` | 行情管线 + 特征加载 | 数据层/特征 | 无下单；特征值正确；日志输出匹配预期 |
| `FixedRebalance` | 撮合流程 | 市价/限价、滑点、手续费、部分成交、订单状态 | 成交笔数、手续费总额、订单生命周期统计 |
| `RiskStress` | 风控规则触发 | `MaxOrderSize`, `PnLDailyLimit`, `StopLoss` 等 | 风控拒单次数、强制平仓记录、策略暂停状态 |
| `SnapshotResume` | 快照与恢复 | 策略状态、共享因子、执行/风险/账簿快照 | 恢复后 PnL/仓位连续；无重复成交；快照文件存在 |
| `EdgeCase` | 异常边界 | 数据缺口、策略异常、无效订单、死信 | 错误事件被捕捉；系统不崩溃；死信记录数量 |

### 9.4 断言与报告
- 每个策略目录包含 `expected.json`（或相似格式）记录期望值，如：
  ```json
  {
    "totalTrades": 4,
    "netPnl": "123.45",
  ...
  ```
- 测试运行器在执行后生成 `actual.json`，比较两者差异（允许运行间容差，如浮点误差）。
- 断言失败输出详细 diff，便于定位问题。

### 9.5 测试执行流程
- CLI 命令示例：`pnpm test:framework` / `npm run test:framework`。
- 步骤：
  1. 清理历史结果目录；
  2. 遍历测试策略目录，读取配置并启动回测；
  3. 回测完成后收集 `ledger`, `risk_events`, `logs`, `metrics` 等；
  4. 运行断言脚本，将结果写入报告（JUnit/JSON/Markdown）；
  5. 返回整体成功/失败状态。
- 可在 CI 中运行，也可本地执行。

### 9.6 快照与数据集
- 测试策略使用固定的输入数据（Parquet/CSV/Mock），保证结果可重复；
- 快照测试：策略在运行过程触发 `pause/snapshot/resume`，断言恢复后与预期一致；
- 有些策略需要多个快照比较（例如 `SnapshotResume`），可在 `expected.json` 中附加 `snapshots` 字段描述快照内容。

### 9.7 扩展与维护
- 新增功能时，增加对应测试策略或扩展现有断言；
- 回归 bug 时，先编写复现策略/断言，再修复 bug，确保问题不再出现；
- 定期回顾测试覆盖面，防止遗漏关键场景。
