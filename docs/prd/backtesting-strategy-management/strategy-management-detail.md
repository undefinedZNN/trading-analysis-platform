...
- 操作：
  - 新建策略：通过弹窗填写基础信息与初始脚本，创建成功后刷新列表并进入脚本版本管理。
  - 编辑策略：支持修改描述、标签等基础信息（策略名称暂不可编辑），脚本版本保持在详情页中管理。
  - 查看详情：打开策略详情页（默认展示脚本版本列表，可对比/编辑脚本版本）。
- 标签数据可通过 `GET /backtesting/strategies/tags` 获取，用于前端筛选与联想。
...
- 脚本输入采用 Monaco 编辑器，默认填充基础模版；保存后打开脚本版本管理抽屉，可直接创建首个脚本版本并刷新列表。
...
- 复制版本：
  ...
  - 支持在页面中直接编辑脚本内容并保存更新。
  - 列表支持展开查看 Schema 字段详情（key、类型、默认值、必填状态等）。
- 删除草稿：
...
- 自定义参数 Schema 预览：
  - 开发者在回测脚本中通过 SDK（如 `defineStrategy`、`parameter.*` 工厂函数）声明参数字段；保存脚本时后端解析导出元数据。
  - 前端以只读形式展示解析结果，字段包含 `key`、`label`、`type`、`component`、`defaultValue`、`required`、`validator`、`enumOptions` 等，便于核对。
  - 提供内置校验与自定义表达式的写法提示，以指导脚本编写。
- 自定义因子 Schema 预览：
  - 与参数 Schema 共用同一套渲染器，字段含义一致，由脚本中的 `factor.*` 声明生成。
  - 支持标记字段类别（系统、脚本）仅用于展示；系统因子列表由平台维护，脚本因子来自代码定义。
- 保存流程：
  - 点击“保存版本”→ 触发代码校验 + SDK Schema 解析 → 成功后生成新版本记录。
  - 弹出保存结果，展示版本号，可立即编辑备注、版本号。
  - 保存成功后返回版本列表并突出显示刚创建的版本。

#### 5.4.1 参数与因子 Schema 注册流程
1. 策略脚本通过 SDK 导出统一入口（示例：`defineStrategy({ parameters, factors, run })`）。
2. 参数使用 `parameter.number`、`parameter.select` 等工厂方法声明字段属性；因子使用 `factor.number`、`factor.custom` 等方法注册。
3. 保存版本时后端在受控环境中加载脚本，读取导出的 `parameters`、`factors` 数据结构并校验合法性。
4. 解析结果写入 `script_versions.parameter_schema` / `script_versions.factor_schema`（JSONB），供前端表单渲染与回测任务运行时校验复用。
5. 运行回测任务时，平台根据 Schema 自动校验用户输入的参数，并提供 SDK API 让策略记录自定义因子值。

### 5.5 版本对比
- 支持在版本列表选中两个版本进行对比。
- 对比内容：
  - 代码 diff（展开为左右对比或高亮行差，页面中支持内联颜色标识）。
  - 参数 Schema diff：新增/删除/修改字段高亮展示（按 key 比对）。
  - 因子 Schema diff：同上。
- 对比弹窗提供基准与对比版本的 Schema 字段预览，便于逐项核对。
- 后端接口：`GET /backtesting/strategies/{strategyId}/script-versions/{versionId}/diff`，通过 `compareVersionId` 或 `compareVersionName` 指定参照版本，返回代码分段 diff 及参数/因子字段的新增、删除、变更列表。
