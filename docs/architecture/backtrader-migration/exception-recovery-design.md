...
- 支持断点续跑功能
- 适用于长时间运行的回测任务
- Worker 崩溃后可以从最近的 Checkpoint 恢复
...
- 平衡存储开销和恢复时间
- Checkpoint 包含：账户状态、持仓、已执行交易、数据索引等

**实现示例**：
```python
checkpoint_interval = 1000  # 每 1000 根K线
for idx in range(start_idx, total_bars):
    cerebro.next()
...
- Worker 定期发送心跳（30秒一次）
- Backend 检测心跳超时（60秒）
- 超时后标记 Worker 为离线，任务状态改为 FAILED
...
async checkWorkerHeartbeats() {
  ...
  for (const worker of workers) {
    const elapsed = now.getTime() - worker.last_heartbeat.getTime();
    
    if (elapsed > timeout) {
      // 标记 Worker 离线，任务失败
      await this.handleWorkerTimeout(worker);
    }
  }
}
```
