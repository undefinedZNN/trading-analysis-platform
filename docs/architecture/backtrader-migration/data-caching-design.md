...
**实现方式**：
- ✅ **LRU 淘汰**：内存不足时自动淘汰最久未使用的数据
- ✅ **Worker 重启时清空**：保证干净的启动状态
- ❌ 不需要 TTL 过期
- ❌ 不需要手动清除
...
**说明**：
- 先用内存缓存
- 后续根据需求升级（多 Worker 共享场景）
- 渐进式升级路径：内存缓存 → Redis（L2）→ 混合缓存（L1 + L2）
